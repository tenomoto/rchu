...
$$
\mathbf{T} = \mathbf{Q}^\dagger\mathbf{A}\mathbf{Q}
$$

ここで、  \mathbf{T}$ は$\boldsymbol\alpha = [\alpha_1,\,\alpha_2,\,\dots,\,\alpha_k]$を対角成分、 $\boldsymbol\beta = [\beta_1,\,\beta_2,\,\dots,\,\beta_k]$ 副対角成分とする $k \times k$ の $ 三重対角行列、
$\mathbf{Q} = [\mathbf{q}_1, \mathbf{q}_2, \dots, \mathbf{q}_k]$ は列ベクトル $\mathbf{q}_i$ を並べた $n\times k$ 行列である。
...
三重対角行列を固有値分解することにより、固有値・固有ベクトルを得る。

...
for (iter in 1: itermax) {
  forward.result <- forward(dt, a, x1, y1, nmax)
  xf <- forward.result$x[tobs]
  yf <- forward.result$y[tobs]
  aa <- adjoint(dt, a, xf, yf)
  ghist[iter] <- sqrt(sum(aa^2))
  if (ghist[iter] < 1e-6) {
    cat("Converged at iteration", iter, "with cost", chist[iter], "\n")
    break
  }
  chist[iter] <- calc.cost(xf, yf, xo, yo)
  if (chist[iter] >= chist[iter-1] && iter > 1) {
    cat("Cost did not decrease at iteration", iter, "with cost", chist[iter], "\n")
    break
  }
  xhist[, iter] <- c(a, x1, y1)
#  cat("iter:", iter, "cost", chist[iter],  "a:", a, "x1:", x1, "y1:", y1, "\n")
#  cat("iter:", iter, "cost", chist[iter], "|g|", ghist[iter], "x1:", x1, "y1:", y1, "\n")
  a <- a - lr * aa[1:6]
  x1 <- x1 - lr * aa[7]
  y1 <- y1 - lr * aa[8]
}
...
```{r}
plot(chist[1:iter], type = "l", lwd = 2, xlab = "Iteration", ylab = "Cost",
     cex.lab = 1.5, cex.axis = 1.5)
```

```{r}
plot(ghist[1:iter], type = "l", lwd = 2, xlab = "Iteration", ylab = "Gradient norm",
     cex.lab = 1.5, cex.axis = 1.5)
```